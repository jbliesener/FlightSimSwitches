...
{
	if (checkInitialized("setNumberOfRows", false)) numberOfRows = rows;
}
void setNumberOfInputs(uint8_t columns)
{
	if (checkInitialized("setNumberOfColumns", false)) numberOfColumns = columns;
}
void setOutputPins(const uint8_t *rowPins)
{
	if (checkInitialized("setRowPins", false)) this->rowPins = rowPins;
}
void setInputPins(const uint8_t *columnPins)
{
	if (checkInitialized("setColumnPins", false)) this->columnPins = columnPins;
}
...
{
	if (checkInitialized("setActiveLow", false)) this->activeLow = activeLow;
}
void setRowsMultiplexed(uint32_t rowsMuxed)
{
	if (checkInitialized("setRowsMultiplexed", false)) this->rowsMuxed = rowsMuxed;
}
void begin();
void loop();
uint32_t *getRowData()
...
{
	this->command = command; this->commandName = command;
};
